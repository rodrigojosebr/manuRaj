...
export default function PreventivePlanDetailPage() {
  ...
  return (
    <div className={css({ display: 'flex', flexDirection: 'column', gap: '6' })}>
      ...
      {/* Delete Modal */}
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Excluir Plano" size="sm">
      ...
      {/* Generate OS Modal */}
      <Modal isOpen={showGenerateOSModal} onClose={() => setShowGenerateOSModal(false)} title="Gerar Ordem de Serviço" size="sm">
        <p className={css({ color: 'gray.600', marginBottom: '4' })}>
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Editar Plano Preventivo" size="lg">
      <form onSubmit={handleSubmit} className={css({ display: 'flex', flexDirection: 'column', gap: '4' })}>
        ...
        <div>
          ...
          <Button type="button" variant="ghost" size="sm" onClick={addChecklistItem} className={css({ marginTop: '2' })}>
            + Adicionar Item
          </Button>
        </div>
